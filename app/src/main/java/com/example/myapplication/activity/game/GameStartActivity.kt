...
class GameStartActivity : BaseActivity(), WebSocketClient.WebSocketCallback {
    ...
    private var selectedMedia: MediaPlayer? = null
...
//        }
    ...
    private fun startCountdown() {
        ...
        countDownJob = CoroutineScope(Dispatchers.Main).launch {
            while (currentIndex < countdownImages.size) {
                // 이미지 업데이트
                gifImageView.setImageResource(countdownImages[currentIndex])
                gifImageView.visibility = View.VISIBLE
            ...
            // 카운트다운 완료 처리
            gifImageView.setImageResource(R.drawable.start)
            gifImageView.scaleType = ImageView.ScaleType.FIT_XY
            gifImageView.visibility = View.VISIBLE
    ...
    private fun playCountdownSound(resid:Int) {
        mediaPlayer?.release()
